import * as test from 'tape';
import { Test } from 'tape';
import { IFatinaPluginBezier } from '../src/core/plugins/bezier';
import { Fatina } from '../src/fatina';
import { get } from '../src/plugins/bezier';

const src = new Fatina();
src.init(false);
const plugin = get();
src.loadPlugin(plugin);
try {
	src.loadPlugin(plugin);
} catch {}
const fatina = src as IFatinaPluginBezier;

test('[Fatina.Bezier] Test Curve', (t: Test) => {
	const obj = {
		position: { x: 0, y: 0 },
		rotation: 0
	};
	fatina.curve(obj, {
		ctr1: { x: 5, y: 10 },
		ctr2: { x: 10, y: 10 },
		to: { x: 20, y: 0 },
		duration: 25
	}).start();

	fatina.update(25);
	t.equal(obj.position.x, 20);
	t.equal(obj.position.y, 0);
	t.end();
});

test('[Fatina.Bezier] Test Curve Quadratic', (t: Test) => {
	const obj = {
		position: { x: 0, y: 0 },
		rotation: 0
	};
	const tween = fatina.curve(obj, {
		ctr1: { x: 5, y: 10 },
		ctr2: { x: 10, y: 10 },
		to: { x: 20, y: 0 },
		duration: 25,
		method: 'quadratic'
	}).start();

	fatina.update(25);
	t.equal(obj.position.x, 20);
	t.equal(obj.position.y, 0);
	tween.kill();
	t.end();
});

test('[Fatina.Bezier] Test Arc', (t: Test) => {
	const obj = {
		position: { x: 0, y: 0 },
		rotation: 0
	};
	const tween = fatina.arc(obj, {
		ctr1: { x: 5, y: 10 },
		to: { x: 20, y: 0 },
		duration: 25
	}).start();

	fatina.update(25);
	t.equal(obj.position.x, 20);
	t.equal(obj.position.y, 0);
	tween.kill();
	t.end();
});

test('[Fatina.Bezier] Test Path', (t: Test) => {
	const obj = {
		position: { x: 0, y: 0 },
		rotation: 0
	};
	const tween = fatina.path(obj, {
		points: [
			{ x: 5, y: 10 },
			{ x: 50, y: 50 },
			{ x: 20, y: 0 }
		],
		method: 'catmull',
		duration: 100
	}).start();
	fatina.update(100);
	t.equal(obj.position.x, 20);
	t.equal(obj.position.y, 0);
	tween.kill();
	t.end();
});

test('[Fatina.Bezier] Test Path Linear', (t: Test) => {
	const obj = {
		position: { x: 0, y: 0 },
		rotation: 0
	};
	fatina.path(obj, {
		points: [
			{ x: 5, y: 10 },
			{ x: 50, y: 50 },
			{ x: 20, y: 0 }
		],
		method: 'linear',
		duration: 100
	}).start();
	fatina.update(100);
	t.equal(obj.position.x, 20);
	t.equal(obj.position.y, 0);
	t.end();
});
