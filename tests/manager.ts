...
test('[Fatina.Manager] Set timescale', (t: Test) => {
  ...
  t.equal(
    previousElapsed + 0.5,
    fatina.elapsed,
    'Check the fatina elapsed time was properly updated with the timescale'
  )
...
test('[Fatina.Manager] Create sequence 2', (t: Test) => {
  const obj = { x: 0 }
  const sequence = fatina.sequence([fatina.delay(1), fatina.tween(obj).to({ x: 1 }, 1)]).start()
...
test('[Fatina.Manager] Check Default Ticker', (t: Test) => {
  ...
  fatina
    .tween(obj)
    .to({ a: 2 }, 5)
    .onUpdate(() => t.equal(++evtOrder, 3))
    .start()
  fatina
    .tween(obj)
    .to({ b: 0 }, 5)
    .onUpdate(() => t.equal(++evtOrder, 4))
    .start()
...
test('[Fatina.Manager] Create cascade tween', (t: Test) => {
  ...
  const tween = fatina
    ...
    .to({ x: 500 }, 500)
    .onComplete(() => {
      fatina.tween(obj).to({ x: 0 }, 500).start()
    })
  tween.start()
