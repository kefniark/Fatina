...
{logLevel:0,safe:!0,smooth:!1,maxFrameDt:50,maxFrameNumber:40,maxDt:500};this.lastTime=this.dt=this.time=0;this.initialized=!1;this.pulse=(a,b)=>f.pulsePreset(this,a,b);this.scale=(a,b)=>f.scalePreset(this,a,b);this.wobble=(a,b)=>f.wobblePreset(this,a,b);this.sonar=(a,b)=>f.sonarPreset(this,a,b);this.shake=(a,b)=>f.shakePreset(this,a,b)}get elapsed(){return this.manager.elapsed}get mainTicker(){this.initialized||this.init();return this.manager}init(a){if(this.initialized)return!1;this.manager||(this.manager=
new b.Ticker,this.manager.start());"undefined"===typeof window||a||(h=k(this.updateLoop.bind(this)));return this.initialized=!0}setTimescale(a){this.init();this.manager.setTimescale(a)}pause(){this.init();this.manager.pause()}resume(){this.init();this.manager.resume()}destroy(){this.manager&&this.manager.kill();h&&n(h);this.initialized=!1}update(a){this.initialized&&this.manager&&(this.manager.tick(a),this.time+=a)}tween(a){a=new m.Tween(a);this.addContext(a);return a}sequence(b){b=new a.Sequence(b);
this.addContext(b);return b}delay(a){a=new g.Delay(a);this.addContext(a);return a}setTimeout(a,b){a=(new g.Delay(b)).onComplete(a);this.addContext(a);return a.start()}setInterval(a,b){a=(new g.Delay(b)).onRestart(a).setLoop(-1);this.addContext(a);return a.start()}addContext(a){this.initialized||this.init();a.setParent(this.manager);0===this.settings.logLevel&&this.settings.safe||a.setSettings(this.settings);this.emitCreated(a)}ticker(){this.initialized||this.init();const a=new b.Ticker,c=a.tick.bind(a);
a.setParent(this.manager,c);this.manager.addTick(c);a.start();this.emitCreated(a);return a}updateLoop(a){this.dt+=a-this.lastTime;this.dt>this.settings.maxDt&&(console.warn(`dt too high ${Math.round(this.dt)}ms. , Capped to ${this.settings.maxDt}ms.`),this.dt=this.settings.maxDt);if(this.settings.smooth){let a=0;for(;0<this.dt&&a<this.settings.maxFrameNumber;){const b=Math.min(this.dt,this.settings.maxFrameDt);this.update(b);this.dt-=b;a++}}else this.update(this.dt),this.dt=0;this.lastTime=a;h=k(this.updateLoop.bind(this))}loadPlugin(a){a.init(this);
this.loadedPlugins.push(a);this.info(2,"Plugin Loaded",a.name)}info(a,b,c){a>this.settings.logLevel||(c?console.log(b,c):console.log(b))}emit(a,b){if(!this.settings.safe)return a(b);try{a(b)}catch(u){console.warn(u)}}emitCreated(a){for(const b of this.eventCreated)this.emit(b,a)}addListenerCreated(a){this.eventCreated.push(a)}removeListenerCreated(a){a=this.eventCreated.indexOf(a);-1!==a&&this.eventCreated.splice(a,1)}setLog(a){this.settings.logLevel=a}setSafe(a){this.settings.safe=a}}d.Fatina=l},
function(c,d,e){function f(a,b){b=b.split(".");for(let c=0;c<b.length-1;c++)a=a[b[c]];return a}function b(a){a=a.split(".");return a[a.length-1]}function g(a,c){const d={};d[b(a)]=c;return d}function a(a,c,d){const l=Object.assign({},{scaleX:"scale.x",scaleY:"scale.y",amplitude:.5,duration:2E3,bounce:5,friction:2,sinX:0},d||{}),e=f(c,l.scaleX),h=b(l.scaleX),g=b(l.scaleY);var m=e[h],k=e[g];return a.tween({}).to({},l.duration).setEasing("inOutCubic").onUpdate((a,b)=>{a=Math.pow(1-b,l.friction);b=b*
l.bounce%l.duration;e[h]=m+Math.sin(l.sinX+b*Math.PI*2)*l.amplitude*a;e[g]=k+Math.sin(b*Math.PI*2)*l.amplitude*a}).onKilled(()=>{e[h]=m;e[g]=k})}Object.defineProperty(d,"__esModule",{value:!0});const m=e(1);d.sonarPreset=function(a,c,d){const e=Object.assign({},{alpha:"alpha",scaleX:"scale.x",scaleY:"scale.y",amplitude:4,duration:2E3},d||{}),h=f(c,e.scaleX),g=f(c,e.alpha),k=b(e.scaleX),n=b(e.scaleY),r=b(e.alpha);var p=h[k],q=h[n];const t=m.easeNames.outCubic;return a.tween({}).to({},e.duration).onUpdate((a,
b)=>{g[r]=1-m.easeNames.inSine(b);h[k]=p+e.amplitude*t(b);h[n]=q+e.amplitude*t(b)}).onKilled(()=>{g[r]=1;h[k]=p;h[n]=q})};d.pulsePreset=function(a,c,d){const e=Object.assign({},{alpha:"alpha",duration:2E3},d||{}),h=f(c,e.alpha);return a.tween(h).to(g(e.alpha,0),e.duration/2).setEasing("inOutQuad").toSequence().append(a.tween(h).to(g(e.alpha,1),e.duration/2).setEasing("inOutQuad")).onKilled(()=>h[b(e.alpha)]=1)};d.scalePreset=a;d.wobblePreset=function(b,c,d){return a(b,c,Object.assign({},{sinX:Math.PI},
d||{}))};d.shakePreset=function(a,c,d){const e=Object.assign({},{posX:"position.x",posY:"position.y",amplitude:1.5,duration:2E3,bounce:10,friction:2},d||{}),h=f(c,e.posX),g=b(e.posX),m=b(e.posY);var k=h[g],n=h[m],p=.5+Math.random(),q=.5+Math.random();return a.tween({}).to({},e.duration).onUpdate((a,b)=>{a=Math.pow(1-b,e.friction);b=b*e.bounce%e.duration;h[g]=k+Math.sin(Math.PI+(b+p)*Math.PI*2)*e.amplitude*a;h[m]=n+Math.sin((b+q)*Math.PI*2)*e.amplitude*a}).onKilled(()=>{h[g]=k;h[m]=n})}},function(c,
d,e){Object.defineProperty(d,"__esModule",{value:!0});class f{constructor(){this.state=0;this.timescale=1;this.duration=this.elapsed=0;this.ticks=new Set;this.newTicks=new Set;this.dt=0}setParent(b,c){this.tickCb=c;this.parent=b}setTimescale(b){this.timescale=b}addTick(b){this.newTicks.add(b)}removeTick(b){this.ticks.delete(b)||this.newTicks.delete(b)}tick(b){1===this.state&&(this.dt=b*this.timescale,0<this.newTicks.size&&(this.newTicks.forEach(b=>this.ticks.add(b)),this.newTicks.clear()),this.ticks.forEach(b=>
b(this.dt)),this.elapsed+=this.dt)}start(){0===this.state&&(this.state=1)}pause(){1===this.state&&(this.state=2)}resume(){2===this.state&&(this.state=1)}kill(){3<=this.state||(this.parent&&this.tickCb&&this.parent.removeTick(this.tickCb),this.state=4)}skip(){}reset(){this.state=0}get isIdle(){return 0===this.state}get isRunning(){return 1===this.state}get isFinished(){return 3<=this.state}get isPaused(){return 2===this.state}}d.Ticker=f},function(c,d,e){Object.defineProperty(d,"__esModule",{value:!0});
c=e(0);class f extends c.BaseTween{constructor(b){super();this.callback=b;this.tickCb=this.tick.bind(this)}tick(b){this.elapsed+=b;this.duration=0;this.callback();this.emitEvent(this.events.update,[b,1]);this.complete()}}d.Callback=f},function(c,d,e){Object.defineProperty(d,"__esModule",{value:!0});const f=e(1);c=e(0);const b=e(3);class g extends c.BaseTween{constructor(a){super();this.prop=[];this.steps=0;this.relative=!1;this.remains=this.v=this.p=0;this.obj=a;this.tickCb=this.tick.bind(this)}init(a){this.obj=
a;this.prop.length=0}validate(){if(!this.obj)throw Error("undefined object");if(!this.parent)throw Error("no ticker");this.ease||(this.ease=f.easeNames.linear);this.check()}check(){this.cf||(this.cf={});this.ct||(this.ct={});for(const a of this.prop)this.f?(this.cf[a]=this.f[a],this.obj[a]=this.f[a]):this.cf[a]=this.obj[a],this.ct[a]=this.relative?this.obj[a]+this.t[a]:this.t[a]}tick(a){if(!(3<=this.state))for(this.remains=a*this.timescale;0<this.remains;){this.elapsed+=this.remains;this.p=Math.max(Math.min(this.elapsed/
this.duration,1),0);this.v=this.ease(this.p);this.yo&&1===(this.yo.original-this.yo.value)%2&&(this.v=1-this.ease(1-this.p));0!==this.steps&&(this.v=Math.round(this.v*this.steps)/this.steps);for(const a of this.prop)this.obj[a]=this.cf[a]+(this.ct[a]-this.cf[a])*this.v;this.events.update&&this.emitEvent(this.events.update,[this.remains,this.p]);if(this.elapsed<this.duration)break;this.remains=this.elapsed-this.duration;if(this.yo&&0!==this.yo.value)this.reverse(),this.resetAndStart(0),this.yo.value--;
else{if(this.loop&&(this.loop.value--,0!==this.loop.value)){this.check();this.resetAndStart(0);continue}this.complete();break}}}from(a){this.f=a;this.updateProp();return this}to(a,b){this.t=a;this.duration=b;this.updateProp();return this}updateProp(){if(this.obj){for(const a in this.t)this.t.hasOwnProperty(a)&&this.prop.push(a);this.prop.filter((a,b,c)=>b===c.indexOf(a))}}setRelative(a){this.relative=a;return this}modify(a,b){for(const c of this.prop)a.hasOwnProperty(c)&&(this.obj[c]+=a[c],b?this.ct[c]+=
a[c]:this.cf[c]+=a[c])}reset(a){if(this.yo){if(1===(this.yo.original-this.yo.value)%2){let a=this.cf;this.cf=this.ct;this.ct=a;a=this.f;this.f=this.t;this.t=a;this.elapsed=Math.round((1-this.elapsed/this.duration)*this.duration*1E3)/1E3}this.yo.value=this.yo.original}super.reset(a)}reverse(){let a=this.cf;this.cf=this.ct;this.ct=a;a=this.f;this.f=this.t;this.t=a;this.elapsed=Math.round((1-this.elapsed/this.duration)*this.duration*1E3)/1E3;3===this.state&&(this.reset(!0),this.start())}yoyo(a){this.yo||
(this.yo={original:0,value:0});this.yo.original=a;this.yo.value=a;return this}setSteps(a){this.steps=a;return this}toSequence(){if(!this.parent)throw Error("parent ticker not defined");return(new b.Sequence).setParent(this.parent).append(this)}setEasing(a){if(!(a in f.easeNames))throw Error(`unknown easing method ${a}`);this.ease=f.easeNames[a];return this}loopInit(){this.elapsed=0}}d.Tween=g},function(c,d,e){Object.defineProperty(d,"__esModule",{value:!0});c=d.EasingType||(d.EasingType={});c.Linear=
"linear";c.InQuad="inQuad";c.OutQuad="outQuad";c.InOutQuad="inOutQuad";c.InCubic="inCubic";c.OutCubic="outCubic";c.InOutCubic="inOutCubic";c.InQuart="inQuart";c.OutQuart="outQuart";c.InOutQuart="inOutQuart";c.InSine="inSine";c.OutSine="outSine";c.InOutSine="inOutSine";c.InCirc="inCirc";c.OutCirc="outCirc";c.InOutCirc="inOutCirc";c.InQuint="inQuint";c.OutQuint="outQuint";c.InOutQuint="inOutQuint";c.InExponential="inExponential";c.OutExponential="outExponential";c.InOutExponential="inOutExponential";
c.InElastic="inElastic";c.OutElastic="outElastic";c.InOutElastic="inOutElastic";c.InBack="inBack";c.OutBack="outBack";c.InOutBack="inOutBack";c.InBounce="inBounce";c.OutBounce="outBounce";c.InOutBounce="inOutBounce"}])});}).call(this || window)
