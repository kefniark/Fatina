...
{logLevel:0,safe:!0,smooth:!1,maxFrameDt:50,maxFrameNumber:40,maxDt:500};this.version="3.0.0-beta.3";this.lastTime=this.dt=this.time=0;this.initialized=!1}get elapsed(){return this.manager.elapsed}get mainTicker(){this.initialized||this.init();return this.manager}init(a){if(this.initialized)return!1;this.manager||(this.manager=new f.Ticker,this.manager.start());"undefined"===typeof window||a||(h=k(this.updateLoop.bind(this)));console.log(" %c Fatina - Tweening library for games ("+this.version+
") https://github.com/kefniark/Fatina ","background: #222; color: #9fbff4; padding: 5px");return this.initialized=!0}setTimescale(a){this.init();this.manager.setTimescale(a)}pause(){this.init();this.manager.pause()}resume(){this.init();this.manager.resume()}destroy(){this.manager&&this.manager.kill();h&&l(h);this.initialized=!1}update(a){this.initialized&&this.manager&&(this.manager.tick(a),this.time+=a)}tween(b){b=new a.Tween(b);this.addContext(b);return b}sequence(a){a=new g.Sequence(a);this.addContext(a);
return a}delay(a){a=new b.Delay(a);this.addContext(a);return a}setTimeout(a,c){a=(new b.Delay(c)).onComplete(a);this.addContext(a);return a.start()}setInterval(a,c){a=(new b.Delay(c)).onRestart(a).setLoop(-1);this.addContext(a);return a.start()}addContext(a){this.initialized||this.init();a.setParent(this.manager);0===this.settings.logLevel&&this.settings.safe||a.setSettings(this.settings);this.emitCreated(a)}ticker(){this.initialized||this.init();const a=new f.Ticker,b=a.tick.bind(a);a.setParent(this.manager,
b);this.manager.addTick(b);a.start();this.emitCreated(a);return a}updateLoop(a){this.dt+=a-this.lastTime;this.dt>this.settings.maxDt/2&&0===this.lastTime?this.dt=0:this.dt>this.settings.maxDt&&(console.warn(`dt too high ${Math.round(this.dt)}ms. , Capped to ${this.settings.maxDt}ms.`),this.dt=this.settings.maxDt);if(this.settings.smooth){let a=0;for(;0<this.dt&&a<this.settings.maxFrameNumber;){const b=Math.min(this.dt,this.settings.maxFrameDt);this.update(b);this.dt-=b;a++}}else this.update(this.dt),
this.dt=0;this.lastTime=a;h=k(this.updateLoop.bind(this))}loadPlugin(a){a.init(this);this.loadedPlugins.push(a);console.log(" %c Fatina - Loaded Plugin "+a.name,"background: #222; color: #9fbff4; padding: 5px");this.info(2,"Plugin Loaded",a.name)}info(a,b,c){a>this.settings.logLevel||(c?console.log(b,c):console.log(b))}emit(a,b){if(!this.settings.safe)return a(b);try{a(b)}catch(n){console.warn(n)}}emitCreated(a){for(const b of this.eventCreated)this.emit(b,a)}addListenerCreated(a){this.eventCreated.push(a)}removeListenerCreated(a){a=
